...
Band 1: "Der Friedhof ohne Namen" (Entscheidungsbuch)
...
# --- Amazon-Rezensions-QR-Code ---------------------------------------------
# TODO VOR VEROEFFENTLICHUNG: hier die ASIN von Band 2 eintragen (kommt nach dem
# KDP-Upload). Solange sie leer ist ("") wird KEIN QR-Code erzeugt, sondern nur
# der Klartext-Hinweis gezeigt. Sobald die ASIN steht -> Build neu laufen lassen.
AMAZON_ASIN = "B0H75HWHFQ"   # Band 2 CYOA (eingetragen 2026-07-01)
# Amazon.de-Direktlink zum Bewertungsformular fuer diese ASIN:
def amazon_review_url(asin):
    return "https://www.amazon.de/review/create-review?asin=" + asin

_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_PROJECT_ROOT = os.path.join(_SCRIPT_DIR, "..")
V2_DIR = os.path.join(_PROJECT_ROOT, "Band2", "CYOA")
GRAPH_FILE = os.path.join(V2_DIR, "graph_v2.yaml")
FRONTMATTER_FILE = os.path.join(_PROJECT_ROOT, "Band2", "CYOA", "frontmatter.md")
OUTPUT_DIR = os.path.join(_PROJECT_ROOT, "Output", "Band2")
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "KDP_Band2_CYOA_Manuskript.docx")

# Illustrationen — Band 2 (15 Stueck). Dateien: Band2/Illustrationen/Illustration N.png
# Keys sind die EXAKTEN Graph-IDs. Bild 1 (Stadtkarte) wird separat als Front-Matter-Seite gesetzt.
ILLUSTRATIONS_DIR = os.path.join(_PROJECT_ROOT, "Band2", "Illustrationen")
ILLUSTRATION_MAP = {
    "P1":  [2],   # Schatten verweigert das Tor
    "P2":  [3],   # Zwei Zonen + auffliegende Stare
    "P3":  [4],   # Der kippende Stein + Silhouette
    "A2":  [5],   # Der Stein mit dem Hund
    "A5":  [6],   # Die erste Blechdose im Schuppen
    "A7":  [7],   # Zwei Geister: Brenner duckt sich vor Voss
    "A10": [8],   # Theo allein am Kapellenfenster
    "B1":  [9],   # Das Archivblatt-Foto (H.B. + G. Voss)
    "B3":  [10],  # Die Liste unter UV-Licht
    "B6":  [11],  # Das unterirdische Registerbuero
    "C1":  [12],  # Doppelschatten im Buerofenster
    "C5":  [13],  # Das Protokollbuch 1886
    "C8":  [14],  # Kloss klagt Voss an (Hoehepunkt)
    "EC1": [15],  # Brenners Befreiung
}
...
def add_title_page(doc):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Ein interaktives Grusel-Abenteuer mit zwölf Enden und einem geheimen Ende")
    run.font.size = Pt(12)
    run.font.italic = True
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(6)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("3 Pfade. 12 Enden. 1 Geheimnis. 13 Entscheidungen.")
    run.font.size = Pt(12)
    run.font.bold = True
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(10)
...
def add_impressum_page(doc):
    ...
    lines = [
        "Die Geistersp\u00fcrer \u2014 Der Friedhof ohne Namen",
        "Ein interaktives Grusel-Abenteuer mit zwölf Enden und einem geheimen Ende",
        "",
        "\u00a9 2026 Benjamin Krug",
...
def add_body_section_break(doc):
    ...
    run = hp_even.add_run("Der Friedhof ohne Namen")
    run.font.name = 'Georgia'
    run.font.size = Pt(8)
    run.font.color.rgb = RGBColor(170, 170, 170)
    run.font.italic = True
...
def add_discoveries_page(doc, sections):
    ...
    places = [
        "Den namenlosen Teil betreten",
        "Die erste Blechdose gefunden",
        "Den Stein mit dem Hund entdeckt",
        "Herrn Kloß zum Reden gebracht",
        "Das unterirdische Registerbüro",
        "Brenner befreit",
        "Alle drei Codewörter gesammelt",
...
def add_notes_pages(doc):
    ...
    # 2 Seiten mit je 14 linierten Zeilen (14 passt sicher auf eine
    # Taschenbuchseite 12,7x20,32 cm, ohne auf eine zweite umzubrechen).
    for page in range(2):
        if page > 0:
            doc.add_page_break()
        for _ in range(14):
...
def add_band2_teaser(doc):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Schatten sieht mehr")
    run.font.size = Pt(16)
    run.font.bold = True
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(6)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Kapitel 1 \u2014 Die Station, die es nicht mehr gibt")
    run.font.size = Pt(13)
    run.font.bold = True
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(14)
    ...
    teaser_lines = [
        "Die Visitenkarte lag noch immer auf dem K\u00fcchentisch.",
        "",
        "\u00bbM. Silber\u00ab, las Theo zum hundertsten Mal. "
        "\u00bbUnd auf der R\u00fcckseite: \u203aNicht alleine.\u2039\u00ab "
        "Er drehte sie um, als st\u00fcnde beim n\u00e4chsten Mal vielleicht mehr darauf.",
        "",
        "Nora sah auf die Karte mit den zehn verbliebenen Markierungen. "
        "Die n\u00e4chste lag tief unter der Altstadt, unter dem Stra\u00dfenniveau \u2014 "
        "eine U-Bahn-Station, stillgelegt seit 1972.",
        "",
        "Daneben, in Frau Silbers kleiner Schrift, ein Satz, "
        "der vorgestern noch nicht dagestanden hatte.",
        "",
        "\u00bbVorsicht\u00ab, las Nora leise. \u00bbEr ist noch ein Kind.\u00ab",
        "",
        "Schatten hob den Kopf. Die Ohren steil. Er sah nicht zur T\u00fcr "
        "und nicht zum Fenster \u2014 er sah schr\u00e4g nach unten, durch den Boden, "
        "irgendwohin weit weg.",
        "",
        "\u00bbTunnel\u00ab, sagte Theo. \u00bbDunkel. Eng. Unter der Erde.\u00ab "
        "Er schluckte. \u00bbIch bin dabei. Aber das sage ich nur ganz leise.\u00ab",
    ]
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Band 3: Schatten sieht mehr")
    run.font.size = Pt(13)
    run.font.bold = True
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(4)
    ...
    series = [
        ("Die Geistersp\u00fcrer",
         ...
         ["Band 1: Das Haus, das fl\u00fcstert",
          "Band 2: Der Friedhof ohne Namen  (dieses Buch)",
...
def _generate_review_qr():
    """Erzeugt (falls ASIN gesetzt) das QR-Code-PNG und gibt den Pfad zurueck,
    sonst None. QR fuehrt direkt zum Amazon-Bewertungsformular."""
    if not AMAZON_ASIN:
        return None
    try:
        import qrcode
    except ImportError:
        print("  WARNUNG: 'qrcode' nicht installiert - QR-Code wird uebersprungen.")
        return None
    url = amazon_review_url(AMAZON_ASIN)
    qr = qrcode.QRCode(box_size=10, border=2,
                       error_correction=qrcode.constants.ERROR_CORRECT_M)
    qr.add_data(url)
    qr.make(fit=True)
    img = qr.make_image(fill_color="black", back_color="white")
    out = os.path.join(OUTPUT_DIR, "review_qr.png")
    img.save(out)
    return out


def add_review_request_page(doc):
    """Rezensions-Bitte (mit QR-Code zur Amazon-Bewertung, falls ASIN gesetzt)."""
    doc.add_page_break()
    for _ in range(2):
        doc.add_paragraph()
    ...
    # --- QR-Code zur Amazon-Bewertung (falls ASIN gesetzt) ---
    qr_path = _generate_review_qr()
    if qr_path:
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(18)
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run("Scann mich — und du bist direkt bei der Bewertung:")
        run.font.size = Pt(11)
        run.font.italic = True
        run.font.name = 'Georgia'
        p.paragraph_format.space_after = Pt(8)

        doc.add_picture(qr_path, width=Cm(3.5))
        qp = doc.paragraphs[-1]
        ...
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run("(Einfach mit der Handy-Kamera auf den Code halten. "
                        "Erwachsene fragen, wenn du Hilfe brauchst.)")
        run.font.size = Pt(9)
        run.font.name = 'Georgia'
        run.font.color.rgb = RGBColor(120, 120, 120)
    else:
        # Fallback ohne QR: Klartext-Hinweis
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(14)
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run("Such einfach auf Amazon nach »Die Geisterspürer "
                        "— Der Friedhof ohne Namen« und hinterlasse dort "
                        "deine Bewertung.")
        run.font.size = Pt(11)
        run.font.italic = True
        run.font.name = 'Georgia'
